...
#METHODE Nous avons utilisé les librairies ggplot2 et reshape2 pour générer une matrice de corrélations entre les variables explicatives (R2, R7, R8, R17, R22, R32).
library(reshape2)
# Sélectionner uniquement les variables numériques explicatives
vars_num <- data[, c("R2","R7","R8","R17","R22","R32")]

# Calculer la matrice de corrélations
corr_matrix <- cor(vars_num)
...
ggplot(corr_long, aes(x = Var1, y = Var2, fill = value)) +
  ...
  scale_fill_gradient2(low = "blue", high = "red", mid = "white",
                       ...
                       name = "Corrélation") +
  theme_minimal() +
  labs(title = "Matrice de corrélations entre variables explicatives",
       x = "", y = "") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))

#RESULTATS Cette visualisation permet de repérer des corrélations fortes (positives ou négatives) entre certaines variables,
